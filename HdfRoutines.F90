...
!RO   Set offsets and element counts
     ...
     &                        hdf_error)
       call h5dwrite_f(dset, H5T_NATIVE_DOUBLE, &
     &   qua(1:nr,xstart(2):xend(2),xstart(3):xend(3)), dims,  &
     &   hdf_error, file_space_id = slabspace, mem_space_id = memspace,  &
     &   xfer_prp = plist_id)
      call h5pclose_f(plist_id, hdf_error)
...
!====================================================================
      ...
      subroutine HdfReadSerialReal2D(filename,dsetname,ndx,ndy,var)
      use hdf5
      implicit none
      character*30,intent(in) :: dsetname,filename
...
!RO   Set offsets and element counts
      ...
      end subroutine HdfReadSerialReal2D
